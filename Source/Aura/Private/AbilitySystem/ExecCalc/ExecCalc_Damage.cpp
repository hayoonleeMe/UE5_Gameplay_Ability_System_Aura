                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : GameplayTags.DamageTypesToResistances)
	{
		...
		// GetSetByCallerMagnitude()에서 못찾으면 0 반환.
		float DamageTypeValue = Spec.GetSetByCallerMagnitude(DamageTypeTag, false) * (100.f - Resistance) / 100.f;
		...
		if (UAuraAbilitySystemLibrary::IsRadialDamage(EffectContextHandle))
		{
			...
			// 5. In Lambda, set DamgaeTypeValue to the damage received from the broadcast.
